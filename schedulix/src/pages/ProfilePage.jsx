// src/pages/ProfilePage.jsx - (For FACULTY)
...
export default function ProfilePage() {
    ...
    const [formData, setFormData] = useState({
        fullName: '',
        department: '',
        designation: '', // New field for user to specify Class/Designation
        subjects: '',
        officeLocation: '' 
    });
    ...
    const loadUserProfile = useCallback(async () => {
        setIsLoading(true);
        try {
            // Use the user from context first, which should be the full DTO
            if (user) {
                setFormData({
                    ...
                    officeLocation: user.currentLocation || '' // Map correctly from the DTO
                });
            } else {
                 // Fallback if context user isn't populated yet
                    ...
                    <form onSubmit={handleProfileUpdate} className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="e.g., Dr. Sonal Balpande"
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={handleInputChange}
                                placeholder="Your Assigned Department"
                                className="h-12 rounded-2xl border-gray-200 bg-gray-50 text-gray-500"
                                disabled
                            />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="designation" className="text-base font-semibold">Designation / Class</Label>
                            <Input
                                id="designation"
                                name="designation"
                                value={formData.designation}
                                onChange={handleInputChange}
                                placeholder="e.g., MCA 1st Year Coordinator"
                                className="h-12 rounded-2xl border-gray-200"
                            />
                        </div>
                        <div className="space-y-2">
                        <Label htmlFor="officeLocation" className="text-base font-semibold">Office / Cabin Location</Label>
                        <Input
                            id="officeLocation"
                            name="officeLocation"
                            value={formData.officeLocation}
                            onChange={handleInputChange}
                            placeholder="e.g., Cabin (304D), 3nd Floor"
                            className="h-12 rounded-2xl border-gray-200"
                        />
                    </div>
                        <div className="space-y-2">
                            <Label htmlFor="subjects" className="text-base font-semibold">Subjects</Label>
                            <Input
                                id="subjects"
                                name="subjects"
                                value={formData.subjects}
                                onChange={handleInputChange}
                                placeholder="e.g., AT, ADSA"
                                className="h-12 rounded-2xl border-gray-200"
                            />
                        </div>
                        <Button
